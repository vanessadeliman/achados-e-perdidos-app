...
class CadastroItemTab extends StatefulWidget {
  final void Function(int) retornar;
  const CadastroItemTab(this.retornar, {super.key});
...
class _CadastroItemTabState extends State<CadastroItemTab> {
  ...
  Widget build(BuildContext context) {
    return BlocListener<RegistrosBloc, RegistrosState>(
      listener: (context, state) {
        if (state is ItemCadastradoComSucesso) {
          widget.retornar(0);
        }
          ...
          child: ListView(
            ...
            children: [
              ...
              TextFormField(
                ...
                validator: validator,
                onTap: () async {
                  dataDaOcorrencia = await showDatePicker(
                    context: context,
                    firstDate: DateTime(2015),
                    lastDate: DateTime(2100),
                  );

                  if (dataDaOcorrencia != null) {
                    setState(() {
                      quando.text = DateFormat(
                        'dd/MM/yyyy',
                      ).format(dataDaOcorrencia!);
                    });
                  }
                },
                decoration: const InputDecoration(
          ...
          onPressed: () async {
            if (chave.currentState!.validate()) {
              final ocorrencia = Ocorrencia(
                usuarioId: context.read<LoginBloc>().sessaoAtiva.id,
                ocorrencia: dataDaOcorrencia!,
                local: onde.text,
                situacao: status.name,
                status: 'perdido',
                item: Item(
                  nome: nome.text,
              ...
              context.read<RegistrosBloc>().add(CadastrarItem(ocorrencia));
            }
          },
